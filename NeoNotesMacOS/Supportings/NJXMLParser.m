...
@interface NJXMLParser () <NSXMLParserDelegate>
...
@implementation NJXMLParser

+ (instancetype)sharedInstance
{
    static dispatch_once_t once;
    static NJXMLParser *sharedInstance;
    dispatch_once(&once, ^{
        
        sharedInstance = [[NJXMLParser alloc] init];
    });
...
{
    NJXMLParser *copy = [[[self class] allocWithZone:zone] init];
...
{
    return [[[NJXMLParser sharedInstance] copy] dictionaryWithParser:parser];
}

+ (NSDictionary *)dictionaryWithXMLData:(NSData *)data
{
    return [[[NJXMLParser sharedInstance] copy] dictionaryWithData:data];
}

+ (NSDictionary *)dictionaryWithXMLString:(NSString *)string
{
    return [[[NJXMLParser sharedInstance] copy] dictionaryWithString:string];
}

+ (NSDictionary *)dictionaryWithXMLFile:(NSString *)path
{
    return [[[NJXMLParser sharedInstance] copy] dictionaryWithFile:path];
}
...
- (NSString *)innerXML
{
    ...
    {
        [nodes addObject:[NJXMLParser XMLStringForNode:childNodes[key] withNodeName:key]];
    }
...
- (NSString *)XMLString
{
    ...
    {
        return [NJXMLParser XMLStringForNode:self withNodeName:[self nodeName] ?: @"root"];
    }
}
