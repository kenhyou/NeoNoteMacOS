...
#define kPenCommMan_Myscript_Enabled @"isMyscriptEnabled"
...
NSString * NJPenRegistrationNotification = @"NJPenRegistrationNotification";
...
@property (nonatomic) NSUInteger disconnectFailCount;
...
//@property (strong, nonatomic) CBUUID *setRtcUuid;
...
//@property (strong, nonatomic) CBUUID *reqCalibrationUuid;
//@property (strong, nonatomic) CBUUID *resCalibrationUuid;
...
//@property (strong, nonatomic) CBCharacteristic *reqCalibrationCharacteristic;
...
@property (nonatomic) NSUInteger disconnectFailCntForRetry;
...
- (instancetype)init
{
    self=[super init];
    if (!self) return nil;
    self.disconnectFailCount = 0;
    self.disconnectFailCntForRetry = 0;
    ...
    //    self.reqCalibrationUuid = [CBUUID UUIDWithString:REQUEST_CALIBRATION_UUID];
    //    self.resCalibrationUuid = [CBUUID UUIDWithString:RESPONSE_CALIBRATION_UUID];
    ...
    //for_calibration
    //self.systemCharacteristics = @[self.penStateDataUuid, self.setPenStateUuid, self.setNoteIdListUuid, self.reqCalibrationUuid, self.resCalibrationUuid];
    ...
    self.penDisconnectedByPen = NO;
    
    self.isNoEligiblePen = NO;
    self.disconnectedByPenBtn = NO;
    self.disConnectedBtnPressed = NO;
...
- (void)setPenConnectionStatus:(NJPenCommManPenConnectionStatus)penConnectionStatus
{
    if(_penConnectionStatus != penConnectionStatus) {
        ...
        if(_penConnectionStatus == NJPenCommManPenConnectionStatusConnected) {
            ...
            self.disConnectedBtnPressed = NO;
            self.disconnectFailCntForRetry = 0;
            [NJSettingStore sharedStore].penColor2 = NSColorFromRGB(vSetting_Pen_Color_Default);
            [NJSettingStore sharedStore].penThickness2 = vSetting_Pen_Thickness_Default;
            [self setPenThickness:[NJSettingStore sharedStore].penThickness];
...
//                [[NJEventLogStore sharedStore] clearQueue];
            });
        }
...
{
    if(!hasPenRegistered) {
        [NJSettingStore sharedStore].askPasswdSetup = YES;
        [NJUtilities saveIntoKeyChainWithPasswd:nil];
    }
    self.penCommParser.passwdCounter = 0;
...
- (BOOL)isMyscriptEnabled
{
    if(![[[[NSUserDefaults standardUserDefaults] dictionaryRepresentation] allKeys] containsObject:kPenCommMan_Myscript_Enabled])
        return YES;
    
    return [[NSUserDefaults standardUserDefaults] boolForKey:kPenCommMan_Myscript_Enabled];
}
- (void)setIsMyscriptEnabled:(BOOL)isMyscriptEnabled
{
    [[NSUserDefaults standardUserDefaults] setBool:isMyscriptEnabled forKey:kPenCommMan_Myscript_Enabled];
    [[NSUserDefaults standardUserDefaults] synchronize];
}
...
{
    [self.penCommParser setDocumentHandler:documentHandler];
}
...
{
    self.penCommParser.shouldSendPageChangeNotification = NO;
}
...
- (void) btStop
{
    if (self.centralManager.state == CBCentralManagerStatePoweredOn) {
        if (self.penConnectionStatus == NJPenCommManPenConnectionStatusScanStarted) {
            ...
            FLog(@"stop scanning by stop searching button");
        }
    }
...
- (void) disConnect
{
    self.disConnectedBtnPressed = YES;
...
{
    FLog(@"disconnect current peripheral %@", self.connectedPeripheral);
    if (self.connectedPeripheral) {
        ...
        /*
         dispatch_async(dispatch_get_main_queue(), ^{
         [[NSNotificationCenter defaultCenter] postNotificationName:NJBTDisconnectedNotification object:nil userInfo:nil];
         });
         */
    }
...
- (void)scan
{
    ...
    if (self.hasPenRegistered) {
        ...
        //                                            options:@{ CBCentralManagerScanOptionAllowDuplicatesKey : @YES }];
        // method 1. use MAC address
        /* method 2. use retrivePeripheral
         NSString *regUuid = self.regUuid;
         NSUUID *nsUUID = [[NSUUID UUID] initWithUUIDString:regUuid];
         ...
         [self.discoveredPeripherals2 addObject:peripheral];
         [self.centralManager connectPeripheral:peripheral options:nil];
...
//            self.isPenSDK2 = NO;
//        } else if ([serviceUUIDs containsObject:self.neoPen2ServiceUuid]){
//            self.isPenSDK2 = YES;
//        }
...
- (void) selectRSSI
{
    ...
    if (self.hasPenRegistered) {
        ...
        FLog(@"[selectRSSI] already registred now trying to connect to pen....");
        if(self.regUuid.length <= 15) {
            // 1. MAC address --> try new method
            for (int i = 0; i < noPeripherals; i++) {
                //NSString *regu = self.regUuid;
                NSString *uid = [self.macArray objectAtIndex:i];
                //NSLog(@"regUuid: %@, array uuid:%@",self.regUuid, uid);
                if ([self.regUuid isEqualToString:uid]) {
                    FLog(@"[selectRSSI] connecting pen by using MAC....");
            ...
            FLog(@"[selectRSSI] connecting pen by using UUID....");
            for (int i = 0; i < noPeripherals; i++) {
        ...
        if ((self.connectedPeripheral == nil) && (_rssiIndex != -1)){
            ...
            if(isEmpty(uid)) {
                ...
                self.isMyscriptEnabled = NO;
                
                if ([[uid uppercaseString] hasPrefix:@"9C7BD2"] ) {
                    self.isMyscriptEnabled = YES;
                    //Megastudy MAC address range : 9C7BD221xxxx, 9C7BD222xxxx,Front Sight: 9C7BD214XXXX, HongShan 9C7BD220XXXX
                    if ([[uid uppercaseString] hasPrefix:@"9C7BD221"] || [[uid uppercaseString] hasPrefix:@"9C7BD222"]
                        ||[[uid uppercaseString] hasPrefix:@"9C7BD214"] || [[uid uppercaseString] hasPrefix:@"9C7BD215"]
                        ||[[uid uppercaseString] hasPrefix:@"9C7BD216"] ||[[uid uppercaseString] hasPrefix:@"9C7BD21F"]
                        ||[[uid uppercaseString] hasPrefix:@"9C7BD22F"]) {
                        self.isMyscriptEnabled = NO;
                    }
                }
            }
...
//            if ([pName isEqualToString:@"NWP-F50"] || [pName isEqualToString:@"NWP-F120"]) {
//                self.isPenSDK2 = YES;
//            } else {
//                self.isPenSDK2 = NO;
//            }
            ...
            if ([serviceUUID isEqualToString:@"19F0"] || [serviceUUID isEqualToString:@"19F1"]) {
                self.isPenSDK2 = YES;
                FLog(@"PenSDK2.0 Pen registered");
            } else {
                self.isPenSDK2 = NO;
                FLog(@"PenSDK1.0 Pen registered");
            }
#else
             self.isPenSDK2 = NO;
#endif
            
            if ([pName hasPrefix:@"MOLESKINE"] || [[uid uppercaseString] hasPrefix:@"9C7BD217"] || [[uid uppercaseString] hasPrefix:@"9C7BD218"]) {
                self.penName = @"Moleskine Pen+";
            }
            
            [self connectPeripheralAt:_rssiIndex];
            FLog(@"registration success uuid %@",uid);
    ...
    FLog(@"[selectRSSI] not found any eligible peripheral....");
    //_penConnectionStatusMsg = @"This pen is not registered.";
    if(self.disconnectFailCount++ >= 1){
        _penConnectionStatusMsg = NSLocalizedString(@"BT_PEN_NOT_CONNECTED", nil);
    } else {
        _penConnectionStatusMsg = @"NULL";
    }
    self.isNoEligiblePen = YES;
    ...
    //retry 3
    if (self.disconnectFailCntForRetry++ < 3)
        [self btStart];
...
//            });
//    }
    FLog(@"Failed to connect to %@. (%@)", peripheral, [error localizedDescription]);
    [self cleanup];
}
...
{
    if (error) {
        FLog(@"Error discovering services: %@", [error localizedDescription]);
        [self cleanup];
        return;
    }
    ...
    for (CBService *service in peripheral.services) {
        FLog(@"Service UUID : %@", [[service UUID] UUIDString]);
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error
{
    ...
    if (error) {
        FLog(@"Error discovering characteristics: %@", [error localizedDescription]);
        [self cleanup];
        return;
    }
...
//                    [self setVersionInfo];
        ...
        for (CBCharacteristic *characteristic in service.characteristics) {
            ...
            if ([self.systemCharacteristics containsObject:characteristic.UUID]) {
                ...
                //                else if([[characteristic UUID] isEqual:self.reqCalibrationUuid]) {
                //                    NSLog(@"reqCalibrationUuid");
                //                    self.reqCalibrationCharacteristic = characteristic;
                ////                    [self.penCommParser reqCalibration];
                //                }
                //                else if([[characteristic UUID] isEqual:self.resCalibrationUuid]) {
                //                    NSLog(@"resCalibrationUuid");
                //                    [peripheral setNotifyValue:YES forCharacteristic:characteristic];
                //                }
            }
            else {
        ...
        for (CBCharacteristic *characteristic in service.characteristics) {
            ...
            if ([self.system2Characteristics containsObject:characteristic.UUID]) {
                if ([[characteristic UUID] isEqual:_penPasswordRequestUuid]) {
                    ...
                    //self.penCommParser.penPasswordResponse = YES;
                }
        ...
        for (CBCharacteristic *characteristic in service.characteristics) {
            ...
            if ([self.offlineCharacteristics containsObject:characteristic.UUID]) {
                if([[characteristic UUID] isEqual:self.requestOfflineFileListUuid]) {
                    ...
                    //                    if (_needRequestOfflineFileList) {
                    //                        [self.penCommParser requestOfflineFileList];
                    //                        _needRequestOfflineFileList = NO;
                    //                    }
                }
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error
{
    ...
    if([ characteristic.UUID isEqual: self.pen2DataUuid] )
    {
        //FLog(@"Received: pen2DataUuid data");
...
//        NSLog(@"Received: id data");
        ...
        FLog(@"Received: offline file info data");
    ...
    //    }
        ...
        FLog(@"Received: penPasswordRequestUuid");
        [self.penCommParser parsePenPasswordRequest:packet withLength:dataLength];
    }
    else if([ characteristic.UUID isEqual: _penPasswordChangeResponseUuid] ) {
        FLog(@"Received: penPasswordResponseUuid");
...
//        }
        ...
        FLog(@"Un-handled data characteristic.UUID %@", [characteristic.UUID UUIDString]);
        return;
    }
}
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateNotificationStateForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error
{
    ...
    if (characteristic.isNotifying) {
        FLog(@"Notification began on %@", characteristic);
    }
    // Notification has stopped
    else {
        // so disconnect from the peripheral
        FLog(@"Notification stopped on %@.  Disconnecting", characteristic);
...
- (void)peripheral:(CBPeripheral *)peripheral didWriteValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error
{
    ...
    if (characteristic == self.pen2SetDataCharacteristic) {
        FLog(@"Pen2.0 Data Write successful");
    }
        ...
        FLog(@"Unknown characteristic %@ didWriteValueForCharacteristic successful", characteristic.UUID);
    }
}
...
- (void)centralManager:(CBCentralManager *)central didDisconnectPeripheral:(CBPeripheral *)peripheral error:(NSError *)error
{
    ...
    if((self.disconnectFailCount++ >= 1) && (self.penDisconnectedByPen == NO)){
        _penConnectionStatusMsg = NSLocalizedString(@"BT_PEN_NOT_CONNECTED", nil);
    } else if (self.penDisconnectedByPen == YES) {
        self.penDisconnectedByPen = NO;
    }
    ...
    if (self.hasPenRegistered && !self.isPenConnected &&
        !self.disconnectedByPenBtn && !self.disConnectedBtnPressed && !self.isNoEligiblePen) {
        if (self.disconnectFailCntForRetry++ < 3) {
            [self btStart];
        } else {
            self.isNoEligiblePen = NO;
...
//- (void)writeReqCalibration:(NSData *)data
//{
//    dispatch_async(bt_write_dispatch_queue, ^{
//        [self.connectedPeripheral writeValue:data forCharacteristic:self.reqCalibrationCharacteristic type:CBCharacteristicWriteWithResponse];
//    });
//}
...
- (void)writePenPasswordResponseData:(NSData *)data
{
    dispatch_async(bt_write_dispatch_queue, ^{
        if (_penPasswordResponseCharacteristic) {
            FLog(@"[PenCommMan -writePenPasswordResponseData] writing data to pen");
...
{
    return [self.penCommParser requestOfflineFileList];
}

- (void)setPenStateWithRGB:(UInt32)color
{
    if (self.isPenSDK2) {
        //[self.penCommParser setPenState2WithRGB:color];
...
{
    if (self.isPenSDK2) {
        //[self.penCommParser setPenState2WithPenPressure:penPressure];
...
{
    if (self.isPenSDK2) {
        //[self.penCommParser setPenState2WithAutoPwrOffTime:autoPwrOff];
...
- (void)setPenStateAutoPower:(unsigned char)autoPower Sound:(unsigned char)sound
{
    if (self.isPenSDK2) {
        ...
        //[self.penCommParser setPenState2AutoPower:autoPower Sound:sound];
    }else{
...
{
    if (self.isPenSDK2) {
        //[self.penCommParser setPenState2WithTimeTick];
...
{
    return [self.penCommParser offlineSyncGAData];
}
