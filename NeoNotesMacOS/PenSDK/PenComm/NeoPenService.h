...
//#define REQUEST_CALIBRATION_UUID   @"2AB3"
...
typedef struct __attribute__((packed)){ //0x05
    ...
    UInt32 colorState;
    unsigned char penPressure;
    //unsigned char reserved[16]; //value ??
} SetPenState2Struct;
...
typedef struct  __attribute__((packed)){ //0x23
    ...
    UInt32 noteId;
    UInt32 pageCnt;
    //UInt32 pageId[NOTE_ID_LIST_SIZE + 1];
...
#define BT_MTU 153
#define UPDATE2_DATA_PACKET_SIZE 2048//1024//128//256//512//1024
...
typedef struct __attribute__((packed)){ //0xB2
    ...
    UInt16 sizeAfterZip;
    //unsigned char fileData[UPDATE2_DATA_PACKET_SIZE]; //??
    UInt8  eof;
